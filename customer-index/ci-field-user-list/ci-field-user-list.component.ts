...
export class CiFieldUserListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loadData();
    this.isEditable=this.authservice.checkPermission(roleType.ADMIN,roleConfig.CIFieldUserCreation) || this.authservice.checkPermission(roleType.MANAGER,roleConfig.CIFieldUserCreation);
  ...
  fetchFilteredData():void{
    ...
    this.restClient
      .getwithParam(environment.FETCH_FIELD_USER_DATA_WITH_FILTER,
        queryParams
      )
      .subscribe({
...
//this.dataSource.data=[]
 ...
 loadData() {
  ...
  showDetails(row: any) {
    ...
    queryParams['fieldUserId'] = row.fieldUserName;

    this.restClient.getwithParam(
      environment.FIELD_USER_CREATION_DETAILS_API,
      queryParams
    )
    .subscribe({
  ...
  exportToExcel(): void {
    ...
    this.restClient
      .downloadusingGet(environment.FIELD_USER_EXPORT_TO_EXCEL,queryParams)
      .subscribe({
        next: (result: any) => {
          this.spinner = false;

          var filename = 'FieldUser Data Export.xlsx';
