...
import { FieldUserAllotmentDetails, FieldUserAllotmentList } from '../ci-field-user-allotment/ci-field-user-allotment.component';

@Component({
  selector: 'app-ci-supervisor-allotment',
  templateUrl: './ci-supervisor-allotment.component.html',
  styleUrls: ['./ci-supervisor-allotment.component.css']
})
export class CiSupervisorAllotmentComponent implements OnInit {
...
searcher:any={
  ...
  ngOnInit(): void {
    ...
    this.loadData();
    this.isDetailsEditable=this.authservice.checkPermission(roleType.ADMIN,roleConfig.CISupervisorAllotment) || this.authservice.checkPermission(roleType.MANAGER,roleConfig.CISupervisorAllotment);
  ...
  fetchFilteredData(){
    ...
    this.restClient
    .getwithParam(
      environment.FETCH_SUPERVISOR_ALLOTMENT_DATA_BY_FILTER,
      queryParams
    )
    .subscribe({
  ...
  loadData():void{
    ...
    this.restClient
      .getwithParam(environment.FETCH_CI_SUPERVISOR_ALLOTMENT, queryParams)
      .subscribe({
        next: (result: PageableResponse) => {
          var content: FieldUserAllotmentList[] = result.data.content as FieldUserAllotmentList[];
 ...
 //navigating the data from list to details page
  showDetails(row: any) {
     ...
     this.restClient.getwithParam(environment.FETCH_SUPERVISOR_ALLOTMENT_DETAILS_API, queryParams)
     .subscribe({
