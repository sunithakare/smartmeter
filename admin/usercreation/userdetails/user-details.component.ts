...
import { DiscomTableIterator } from './interface/DiscomTableIterator';
...
export class UserDetailsComponent implements OnInit {
  ...
  rolesFilterStr: FormControl = new FormControl();
  rolesListFiltered: RoleList[] = [];
  ...
  ngOnInit(): void {
    ...
    let discomFGArray = arrayResposne.map(this.DiscomDataAsFormGroup, {
    ...
    this.dataSource = new MatTableDataSource(
      (this.userForm.get('discomDataList') as FormArray).controls
    );

    this.fetchRoleTypeList();
    this.getAllState();

  this.selectedDiscomsList= [];
  ...
  addNewDiscom() {
    if (this.userForm.value.discomDataList.length > 0) {
      var lastItem =
        this.userForm.value.discomDataList[
          this.userForm.value.discomDataList.length - 1
        ];
    ...
    var today = new Date();
    var dateAfterTwoMonths: Date = new Date(
      today.setMonth(today.getMonth() + 2)
    );
    var fg: FormGroup = this.DiscomDataAsFormGroup(new DiscomData());
    fg.controls['end'].setValue(dateAfterTwoMonths);

    (<FormArray>this.userForm.get('discomDataList')).push(fg);
  ...
  checkUserNameAlreadyExists() {
    ...
    this.restClient
      .getwithParam(environment.CHECK_USER_NAME_API, queryParams)
      .subscribe({
        next: (result: APIResponse) => {
  ...
  ngOnChanges() {
    ...
    if (
      ...
      this.discomArray = [];
      this.userdetails.discomDataList.forEach((discom, index) =>{

        this.discomArray.push({ discomList: [] });
        this.selectedDiscomsList.push(discom.discomName);
      }
      );

      this.userdetails.discomDataList.forEach((discom, index) =>
        this.getDiscomDataForState(discom.state, index)
      );
      this.dataSource = new MatTableDataSource(
        (this.userForm.get('discomDataList') as FormArray).controls
      );
    ...
    this.restClient
      ...
      .subscribe({
        next: (result: APIResponse) => {
          ...
          var allRoleList = result.data as RoleList[];
          this.rolesListFiltered =[];
          for (let index = 0; index < allRoleList.length; index++) {
            const element = allRoleList[index];
            var exists =this.asignedRoles.find(rec=>{
              return (rec!.roleType+rec!.roleName )==(element!.roleType+element!.roleName )
            });
             if (!exists) {
              this.rolesListFiltered .push(element);
             }
          }
  ...
  getAllState() {
    this.spinner = true;
    this.restClient.getResponse(environment.FETCH_ALL_STATES_API).subscribe({
      next: (result: APIResponse) => {
  ...
  selectedDiscomChange(event: MatSelectChange, index: number) {
    if (this.selectedDiscomsList.length>=index) {
      this.selectedDiscomsList[index]=event.value;
    }else{
      this.selectedDiscomsList.push(event.value);
    }
  ...
  getDiscomDataForState(stateName: string, index: number) {
    ...
    this.restClient
      .getwithParam(environment.FETCH_DISCOMS_IN_STATES_API, queryParams)
      .subscribe({
        next: (result: APIResponse) => {
  ...
  getFormGroup(userData: UserData): FormGroup {
    ...
    let discomFomm = userData.discomDataList.map(this.DiscomDataAsFormGroup, {
      isEditable: this.isEditable,
    });
    const fg = new FormGroup({
      firstName: new FormControl(
        { value: userData.firstName, disabled: !this.isEditable },

        [Validators.required, Validators.maxLength(60)]
      ),
      lastName: new FormControl(
        { value: userData.lastName, disabled: !this.isEditable },
        [Validators.required, Validators.maxLength(60)]
      ),
      orgName: new FormControl(
        { value: userData.orgName, disabled: !this.isEditable },
        [Validators.required, Validators.maxLength(100)]
      ),
      employeeId: new FormControl(
        { value: userData.employeeId, disabled: !this.isEditable },
        [Validators.required]
      ),
      email: new FormControl(
        { value: userData.email, disabled: !this.isEditable },
        [Validators.required]
      ),
      phonoNo: new FormControl(
        { value: userData.phonoNo, disabled: !this.isEditable },
        [Validators.required, Validators.pattern('[6-9]\\d{9}')]
      ),
      userId: new FormControl(
        { value: userData.userId, disabled: !this.isEditable },
        [Validators.required]
      ),
      active: new FormControl({
  ...
  DiscomDataAsFormGroup(discomData: DiscomData): FormGroup {
    const fg = new FormGroup({
      discomName: new FormControl(
        { value: discomData.discomName, disabled: !this.isEditable },
        Validators.required
      ),
      state: new FormControl(
        { value: discomData.state, disabled: !this.isEditable },
        Validators.required
      ),
      userDefault: new FormControl(
        { value: discomData.userDefault, disabled: !this.isEditable },
        Validators.required
      ),
      start: new FormControl(
        { value: discomData.start, disabled: !this.isEditable },
        Validators.required
      ),
      end: new FormControl(
        { value: discomData.end, disabled: !this.isEditable },
        Validators.required
      ),
    });
    return fg;
  ...
  createApprover(): void {
    this.router
      .navigate(['ltapp/admin/approverdetails'], {
        state: { userName: this.userForm.getRawValue().userId },
      })
      .then((value) => {
  ...
  rolesFilter(event: any) {
    var search: string = event.target.value as string;
    if (search === '') {
      this.rolesListFiltered = this.rolesList;
    } else {
      this.rolesListFiltered = this.rolesList.filter(function (item) {
        return (
          item.roleName!.toLowerCase().indexOf(search.toLowerCase()) >= 0 ||
          item.roleDesc!.toLowerCase().indexOf(search.toLowerCase()) >= 0 ||
          item.roleType!.toLowerCase().indexOf(search.toLowerCase()) >= 0
        );
      });
    }
  }

  canDisplayDiscom(input:string,index:number):boolean{
   var indexOfDiscom= this.selectedDiscomsList.indexOf(input);
    return indexOfDiscom<0 || indexOfDiscom==index ;
  }
}
