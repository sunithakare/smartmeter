    ...
    this.restClient
      ...
      .subscribe({
        next: (result: APIResponse) => {
          ...
          allApplicationArray.sort((a:ConfigCode,b:ConfigCode)=>{
return this.applicationSorting.indexOf(a.shortCode)-this.applicationSorting.indexOf(b.shortCode);
          });
  ...
  fetchDiscomZone(event:MatSelectChange): void {
    this.spinner = true;
    var queryParams: any = {};
  ...
  fetchZoneCity(event:MatSelectChange): void {
    this.spinner = true;
    var queryParams: any = {};
  ...
  fetchCityCircle(event:MatSelectChange): void {
    this.spinner = true;
    var queryParams: any = {};
  ...
  appRequiredChanged(event:MatCheckboxChange,element:FormGroup ,appName:string){
const vpnReqApp=['MDMs',
'HES',
'HES Dashboard',
'Operational Dashboard',
'BI Analytics']
...
if (event.checked) {
  if (vpnReqApp.indexOf(appName)>=0 && !this.applicationAccess.getRawValue()[0].isRequired && this.applicationDetails.getRawValue().existingVPNName=='') {
    alert("VPN Access Need to be selected for this Application");
    // event.source.checked=false;
    element.controls["isRequired"].setValue(false);
  }
}
  ...
  changedLANMacInput():void{
    if(this.VPNDetails.controls["lanMACAddress"].value){
      const inputWithourColon = this.VPNDetails.controls["lanMACAddress"].value.replace(new RegExp("-", 'g'), "");
      let blocks = inputWithourColon.match(/.{1,2}/g);
      let formatedMac = blocks.shift();
      for (let block of blocks){
       formatedMac = formatedMac + "-" + block;
      }
      this.VPNDetails.controls["lanMACAddress"].setValue(formatedMac);
    }
  }
  changedWIFIMacInput():void{
    if(this.VPNDetails.controls["wifiMACAddress"].value){
      const inputWithourColon = this.VPNDetails.controls["wifiMACAddress"].value.replace(new RegExp("-", 'g'), "");
      let blocks = inputWithourColon.match(/.{1,2}/g);
      let formatedMac = blocks.shift();
      for (let block of blocks){
       formatedMac = formatedMac + "-" + block;
      }
      this.VPNDetails.controls["wifiMACAddress"].setValue(formatedMac);
    }
  }
  ...
  saveData() {
    ...
    this.restClient
      .post(environment.UAM_EDIT_FORM_API+"/"+this.refId, requestBody)
      .subscribe({
        next: (result: APIResponse) => {
          this.dataSaved.emit(1);
          let snackBarRef = this.snackBar.open(
