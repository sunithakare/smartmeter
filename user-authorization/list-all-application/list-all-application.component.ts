...
export class ListAllApplicationComponent implements OnInit {
  ...
  statusList: string[] = [];
  statusListConstant: string[] = ["Submitted" ,"Approved 1" ,"Approved 2" ,"Approved 3" ,"Approved 4" ,"Approved" ,"Rejected","Completed"];
      ...
      .subscribe({
        next: (result: PageableResponse) => {
          ...
          for (const iterator of content) {
            if(iterator.status=="In Progress" || iterator.status=="Awaiting Approval"){
              iterator.status="Submitted";
            }
            if(iterator.status=="Approved U1"){
              iterator.status="Approved 1";
            }
          }
...
generateFormData(formData: Register):UAMInputForm {
  ...
  var applicationAccess = this.formBuilder.array(formData.applicationRequested.map(this.applicationAsFormGroup,{listview:this.listview}));
  ...
  var remarksData=new FormGroup({
    userRemarks: new FormControl( { value: formData.remarksData.userRemarks, disabled: false }),
     spocRemarks: new FormControl({ value: formData.remarksData.spocRemarks, disabled: false }
    ),
});
...
exportToExcel():void{
  ...
  this.spinner = true;
  this.restClient
      .downloadusingGet(environment.UAM_APPROVER_EXPORT_TO_EXCEL_API,{})
      .subscribe({
        next: (result: any) => {

          this.spinner = false;

            var filename="My Approval List.xlsx";
