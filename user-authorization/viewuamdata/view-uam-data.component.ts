...
export class ViewUamDataComponent implements OnInit {
  ...
  constructor(
    ...
    this.dataEditable=this.authservice.checkPermission(roleType.ADMIN,roleConfig.UAM_EDIT) || this.authservice.checkPermission(roleType.MANAGER,roleConfig.UAM_EDIT);
  ...
  ngOnChanges() {
    if (this.inputForm != null && this.inputForm != undefined) {
      ...
      this.remarksData = this.inputForm.remarksData;
      ...
      if (this.applicationDetails.getRawValue().status == 'Completed' || this.applicationDetails.getRawValue().status == 'Approved') {
        this.applicationAccess.disable();
      }
      // if (this.applicationDetails.getRawValue().status == 'Approved' && !this.listview) {
      //   for (let index = 0; index < this.applicationAccess.length; index++) {
      //     this.applicationAccess.at(index).get('start')?.enable();
      //     this.applicationAccess.at(index).get('end')?.enable();
...
editData():void{
  ...
  this.locationDetails.disable();
  this.applicationDetails.disable();
  this.applicationDetails.get('userType')?.disable();
  this.applicationDetails.get('requestFor')?.disable();
  this.applicationDetails.get('existingVPNName')?.enable();
...
editSavedData(i:number):void{
  ...
  submitData(approveReject: string): void {
    ...
    if (approveReject == 'Approved') {
      var isAllowedToApprove = this.applicationAccess
        .getRawValue()
        .find((item) => {
          return item.status == 'Pending';
        });
      if (isAllowedToApprove) {
        alert('One Or More Applciation Access is Not Approved!!');
        return;
      }
      var totalrejected = 0;
      for (const iterator of this.applicationAccess.getRawValue()) {
        if (iterator.status == 'Reject') {
      ...
      if (
        totalrejected > 0 &&
        totalrejected == this.applicationAccess.getRawValue().length
      ) {
        alert('All Applciation Access is Rejected!!');
        return;
      }
    }
    ...
    const dialogRef = this.dialog.open(ApprovalDialogComponent, {
      ...
      // height: '50%',
      data: {
        masterData: this.applicationAccess.getRawValue(),
        remarks: '',
  ...
  saveResourceData(): void {
    ...
    // console.log(JSON.stringify(requestBody));
    this.restClient
      .post(environment.UAM_SPOC_SAVE_API + '/' + this.refId, requestBody)
      .subscribe({
        next: (result: APIResponse) => {
          let snackBarRef = this.snackBar.open(
            'Details Submited SuccessFully',
  ...
  spocDetailsAsFormGroup(spocData: SpocDetails): FormGroup {
    const fg = new FormGroup({
      ...
      securityGroups: new FormControl({
        value: spocData.securityGroups,
        disabled: false,
      }),
      systemDestinationIpSubnets: new FormControl({
        value: spocData.systemDestinationIpSubnets,
        disabled: false,
      }),
      serviceSshHttps: new FormControl({
        value: spocData.serviceSshHttps,
        disabled: false,
      }),
      customPorts: new FormControl({
        value: spocData.customPorts,
        disabled: false,
      }),
    });
    return fg;
  ...
  public savePDF(): void {
    ...
    domtoimage.toPng(DATA!, options).then((dataUrl) => {
      ...
      domtoimage.toPng(TableData!, options).then((dataUrl) => {
        PDF.addPage();
        PDF = PDF.setPage(2);
        PDF.addImage(dataUrl, 'PNG', 0, 0, 290, 134.12);
        ...
        domtoimage.toPng(remarksData!, options).then((dataUrl) => {
          PDF.addPage();
          PDF = PDF.setPage(3);
          PDF.addImage(dataUrl, 'PNG', 0, 0, 300, 184.12);
